...
Public Class crate_receipt
    ...
    Private Sub TextBox3_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles TextBox3.KeyUp
        If e.KeyCode = Keys.Enter Then
            ...
            If Val(TextBox6.Text) >= 0 Then
                ...
                abc1.Parameters.AddWithValue("@dr", SqlDbType.Int).Value = dr
                abc1.Parameters.AddWithValue("@cr", SqlDbType.Int).Value = Val(TextBox3.Text)
            ...
            Else
                MsgBox("No. Of Crate Receipt Is going More than Prv. Crate Balance ")
                Exit Sub
            End If
            TextBox2.Text = ""
            TextBox3.Text = ""
    ...
    Public Sub get_balance()
        ...
        sum_dr = 0
        sum_cr = 0
        abc = New SqlCommand("select sum(dr) from crateledger where companyid=" & companyid & " and  customerid=" & customerid & " and crateid=" & crateid)
        Dim xyz(0), xyz1(0) As String
        a7.array_list(abc, xyz, 0)
        sum_dr = xyz(0)
        abc1 = New SqlCommand("select sum(cr) from crateledger where companyid=" & companyid & "  and customerid=" & customerid & " and crateid=" & crateid)
    ...
    Public Sub list_add()
        ...
        Dim crb As Integer
        crb = 0
        abc = New SqlCommand("select a.cname,b.cr,b.voucherid from customer a,crateledger b where a.customerid=b.customerid and b.crateid=@crateid and b.companyid=@companyid and b.cr>@cr and b.date=@date and b.particular=@particular")
        abc.Parameters.AddWithValue("@companyid", SqlDbType.Int).Value = companyid
        abc.Parameters.AddWithValue("@crateid", SqlDbType.Int).Value = crateid
        abc.Parameters.AddWithValue("@cr", SqlDbType.Int).Value = crb
        abc.Parameters.AddWithValue("@date", SqlDbType.DateTime).Value = DateTimePicker1.Value.Date
        abc.Parameters.AddWithValue("@particular", SqlDbType.VarChar).Value = "RECEIPT"
