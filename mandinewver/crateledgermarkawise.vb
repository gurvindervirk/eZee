...
Public Class Crateledgermarkawise
    ...
    Private Sub TextBox3_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles TextBox3.TextChanged
        ...
        Dim abc As SqlCommand
        abc = New SqlCommand("select cname from crate where companyid=@companyid and cname like @cname + '%'order by cname")
        abc.Parameters.AddWithValue("@companyid", SqlDbType.Int).Value = companyid
    ...
    Public Sub Show_ledger()
        opening_balance()
        Dim abc As SqlCommand
        abc = New SqlCommand("select a.date,b.cname,a.dr,a.cr,a.particular from crateledger a,crate b where a.customerid=@pid and date>=@dt1 and date<=@dt2 and a.crateid=@crateid and a.crateid=b.crateid order by a.date,a.particular")
        abc.Parameters.Add("@dt1", SqlDbType.DateTime).Value = DateTimePicker1.Value.Date
        abc.Parameters.Add("@dt2", SqlDbType.DateTime).Value = DateTimePicker2.Value.Date
        abc.Parameters.Add("@pid", SqlDbType.Int).Value = customerid
    ...
    Public Sub Opening_balance()
        Dim abc As SqlCommand
        abc = New SqlCommand("select sum(dr),sum(cr)from crateledger where customerid=@pid and crateid=@crateid and date<@dt1")
        abc.Parameters.Add("@dt1", SqlDbType.DateTime).Value = DateTimePicker1.Value.Date
        abc.Parameters.Add("@pid", SqlDbType.Int).Value = customerid
        abc.Parameters.Add("@crateid", SqlDbType.Int).Value = crateid
        Dim xyz(1) As String
    ...
    End Sub
    Public Sub Crate_id_no()
        Dim itemname As String
        itemname = TextBox3.Text
        crateid = a7.Get_crate_id(companyid, itemname)
    End Sub
